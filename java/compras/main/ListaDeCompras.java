...
public class ListaDeCompras {
	...
	*/
	...
	private static ArrayList<String> ordenOriginal (ArrayList<String> listaOrig) {
		ArrayList<String> result = new ArrayList<>();
		int i = listaOrig.size()-1;
		while (i != -1) {
			result.add(listaOrig.get(i));
			i--;
		}
		return result;
	}
	...
	private static boolean responderAEntrada(String entrada) {
		...
		if (comando.compareToIgnoreCase(AGREGAR.opcion()) == 0) {
			...
			for (String pd : ordenOriginal(listaCompras)) {
